...
const Orders = () => {
  ...
  const load = async () => {
    if (!user) return;
    const { data } = await (supabase as any).from("orders").select("id,buyer_id,seller_id,amount,quantity,status,provider,mpesa_receipt,created_at,location,pickup_station,delivery_method,delivery_address,tracking_latitude,tracking_longitude,tracking_updated_at,delivered_at,refund_status,listings(title,image_url)").or(`buyer_id.eq.${user.id},seller_id.eq.${user.id}`).order("created_at", { ascending: false });
